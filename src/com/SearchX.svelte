...
<Wrapper id="x-search">
	<HeaderButton icon="apps" on:blur={blurSearchBar} click={toggleMenu} touch />
	<Textfield bind:input={xInputRef} bind:floatingLabel={xLabelRef} bind:lineRipple={xLineRippleRef}>
		<FloatingLabel bind:this={xLabelRef} for="input-manual-a">{xLabel}
		</FloatingLabel>
		<div>
			<FloatingLabel bind:this={xLabelRef} for="input-manual-a">
				<!--{#if (xValue.length==0)}-->
		...
		<Input bind:this={xInputRef} bind:value={xValue} on:focus={focusSearchBar} on:blur={blurSearchBar}
			on:mouseenter={mouseEnterSearchBar} id="input-manual-a" aria-controls="helper-text-manual-a"
